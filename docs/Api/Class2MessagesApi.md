...
```php
forwardMessage($forward_message_request): \OpenAPI\Client\Model\SendMessageStatus
```
...
```php
getMessages($last_message_number, $last, $chat_id, $limit, $min_time, $max_time): \OpenAPI\Client\Model\Messages
```
...
```php
sendLink($send_link_request): \OpenAPI\Client\Model\SendMessageStatus
```
...
```php
sendPTT($send_ptt_request): \OpenAPI\Client\Model\SendMessageStatus
```
...
```php
sendVCard($send_v_card_request): \OpenAPI\Client\Model\SendMessageStatus
```
